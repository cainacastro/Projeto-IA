...
for episode in range(NUM_EPISODES):
    ...
    for t in range(PASSOS_POR_EPISODIO): 
        ...
        if len(replay_buffer) > BATCH_SIZE:
            states, actions, sampled_rewards, next_states, dones = replay_buffer.sample(BATCH_SIZE)
            ...
            expected_q_values = sampled_rewards.to(DEVICE) + (1 - dones.to(DEVICE)) * GAMMA * next_q_values
    ...
    episode_rewards.append(pontos_game) 
    ...
    if (episode + 1) % 1 == 0:
        ...
        plt.xlabel('Episódio')
        plt.ylabel('Pontos')
        plt.title(f'Pontos por Episódio (Episódio {episode + 1})')
        plt.pause(0.01)
...
plt.xlabel('Episódio')
plt.ylabel('Pontos')
plt.title('Pontos por Episódio (Final)')
plt.savefig(f'{MODEL_DIR}/rewards_final.png')
plt.show()